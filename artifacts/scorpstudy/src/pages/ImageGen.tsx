...
import { ImageIcon, Loader2, Save, Download, RefreshCw, Wand2, Sparkles } from "lucide-react";
        ...
        <Card className="border-slate-200">
          <CardContent className="p-5 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {SUGGESTIONS.map((s) => (
                  <button key={s} onClick={() => setPrompt(s)}
                    className="text-xs px-2.5 py-1 rounded-full border border-slate-200 text-slate-600 hover:border-fuchsia-400 hover:text-fuchsia-700 hover:bg-fuchsia-50 transition-colors">
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-3 items-center justify-between">
              ...
              <Button onClick={handleGenerate} disabled={isGenerating || !prompt.trim()} className="bg-fuchsia-600 hover:bg-fuchsia-700 gap-2 shrink-0">
              ...
              <div className="p-4 border-t border-slate-100 bg-white space-y-2">
                <div className="flex justify-between items-start gap-3 flex-wrap">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <Button variant="outline" size="sm" onClick={handleRetry} disabled={isGenerating} className="gap-1.5 text-xs">
                ...
                {imageLoaded && (
                  <Badge variant="secondary" className="text-[10px]">
                    Style: {STYLES[style]?.label ?? style}
                  </Badge>
                )}
              </div>
            </>
